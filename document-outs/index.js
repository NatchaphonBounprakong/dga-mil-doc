...
var app = new Vue({
    ...
    methods: {
        filter() {
            this.organizationFiltered = [];
            this.organizations.forEach(x => {
                if (x.Name.toUpperCase().indexOf(this.organizationSearch.toUpperCase()) != -1) {
                    this.organizationFiltered.push(x)
                }
            });
            ...
            if (this.errors.length == 0) {
                ...
                if (confirm("ต้องการส่งหนังสือเวียนใช่หรือไม่")) {
                    ...
                    for (i = 0; i < this.selectedOrganization.length; i++) {
                        this.selectedOrganizationId.push(this.selectedOrganization[i].Id)
                    }
